...
import { useCountries, useCurrencies, type CurrencyDto } from '@/features/lookups/api';
...
/**
 * The currency list. Creating and editing happen on their own pages rather than in a dialog — the
 * form carries a code, a symbol, a name in two languages and a country mapping across the whole
 * catalogue, which is more than a popup should hold.
 */
export function CurrenciesTab({ caps }: { caps: LookupCaps }) {
  const { t } = useTranslation();
  const navigate = useNavigate();
  const location = useLocation();
  const tab = useLookupTab<CurrencyDto, never>('currencies');
  const list = useCurrencies(tab.params);
    ...
    <div className="space-y-4">
      ...
      <DataTable
        ...
        toolbar={
          caps.canCreate ? (
            <NewButton
              label={t('lookups.createTitle', { entity: t('lookups.currencies') })}
              onClick={() => navigate('/lookups/currencies/new')}
            />
          ) : null
        }
        columns={[
          ...
          { key: 'symbol', header: t('lookups.symbol'), render: (row) => row.symbol },
          ...nameColumns<CurrencyDto>(t),
          {
            key: 'countries',
          ...
          actionColumn<CurrencyDto>(
            t,
            caps,
            // The row travels with the navigation, so the form opens without a second fetch.
            (row) => navigate(`/lookups/currencies/${row.id}/edit`, { state: { currency: row } }),
            tab.setToDelete,
            (row) => row.code,
          ),
        ]}
